...
unsigned char *tga_offset;

unsigned char *readTGA(unsigned char *name) {
    tga_offset = FindFileTAR(name);
    return tga_offset;
}
unsigned int *tga_parse(unsigned char *ptr, int size)
{
    ...
    data = (unsigned int*)page_allocator((w*h+2)*sizeof(unsigned int));
    if(!data) return 0;
 
    switch(ptr[2]) {
        case 1:
            if(ptr[6]!=0 || ptr[4]!=0 || ptr[3]!=0 || (ptr[7]!=24 && ptr[7]!=32)) { ; return 0; }
            for(y=i=0; y<h; y++) {
                k = ((!o?h-y-1:y)*w);
                for(x=0; x<w; x++) {
            ...
            break;
        case 2:
            if(ptr[5]!=0 || ptr[6]!=0 || ptr[1]!=0 || (ptr[16]!=24 && ptr[16]!=32)) {  return 0; }
            for(y=i=0; y<h; y++) {
...
void DrawImageTGA(int x, int y, unsigned char *offset) {
    unsigned int *imagedata = tga_parse(offset, 522258);

    printf("Width: ", -1, -1, -1);
    printf(itoa(imagedata[0]), -1, -1, -1);
}